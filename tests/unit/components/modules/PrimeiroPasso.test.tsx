...
describe("Primeiro Passo — smoke de interação", () => {
    ...
    it("conclui a etapa 1, persiste no localStorage e retoma após remontar", async () => {
        ...
        const { unmount } = render(<PrimeiroPasso />);
    ...
    it("mostra o plano final quando as 10 etapas estão concluídas", async () => {
        ...
        window.localStorage.setItem(STORAGE_KEY, JSON.stringify(tudo));
    ...
    it("recomeça do zero limpando o localStorage após confirmação", async () => {
        window.localStorage.setItem(STORAGE_KEY, JSON.stringify({ "1": { __ok: true } }));
