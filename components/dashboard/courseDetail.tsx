...
export default function CourseDetail({ courseId, onBack, onCourseDeleted }: Props) {
    ...
    const [filteredAlunos, setFilteredAlunos] = useState<UserCursoProps[]>([]);
    // Filtra alunos disponíveis baseado na busca
   useEffect(() => {
  if (!searchAluno) {
  ...
  const filtered = alunosDisponiveis.filter((u) =>
    (u.display_name || u.email)
      .toLowerCase()
      .includes(searchAluno.toLowerCase())
  );
  setFilteredAlunos(filtered);
}, [searchAluno, alunosDisponiveis]);
    ...
    const handleAddAluno = async () => {
        if (!novoAlunoId.trim()) return alert("Selecione um aluno!");
    ...
    const handleRemoveAluno = async (matriculaId: string) => {
        ...
        if (!ok) return;
        const sucesso = await removeAlunoDoCurso(matriculaId );
        if (sucesso) {
            setAlunos((prev) => prev.filter((a) => a.Aluno !== matriculaId));
        } else {
    ...
    return (
        <div>
            ...
            <div className="bg-white p-4 rounded shadow mb-6">
                ...
                {/* Adicionar novo aluno via select */}
                <div className="mt-4 flex gap-2 flex-col">
                    <div className="mb-4">
                        ...
                        {
                            <ul className="border rounded p-2 max-h-40 min-h-40 overflow-y-auto bg-white shadow">
                                {filteredAlunos.map((u) => (
                                    <li
                                        key={u.id}
                                        className="py-1 px-2 hover:bg-gray-100 cursor-pointer"
                                        onClick={() => {
                                            setNovoAlunoId(u.id);
                                            setSearchAluno(u.display_name); // limpa a busca após selecionar
                                            setFilteredAlunos([]);
                                        }}
                                    >
                                        {u.display_name || u.email}
