...
const filteredAlunos = alunosDisponiveis.filter((u) =>
  (u.display_name || u.email).toLowerCase().includes(searchAluno.toLowerCase())
);
  // =========================
    ...
    <ul className="border rounded p-2 max-h-40 overflow-y-auto bg-white shadow">
      {filteredAlunos.map((u) => (
        <li
          ...
          onClick={() => {
            setNovoAlunoId(u.id);
            setSearchAluno(""); // limpa a busca após selecionar
            setFilteredAlunos([]);
          }}
        >
